...
```{r}
plot_pattern(synthetic_leiden)
plot_flux(synthetic_leiden)
```
...
In the next steps we are going to impute `rrsyst` and `rrdiast` under two scenarios: MAR and MNAR. We will use the delta adjustment technique described in paragraph 7.2.3 in Van Buuren [-@van2012flexible]
...
See Van Buuren [-@van2012flexible, pp.186] for more information.
...
Sensitivity analysis is an important tool for investigating the plausibility of the MAR assumption. We again use the $\delta$-adjustment technique described in Van Buuren [-@van2012flexible], p. 185] as an informal, simple and direct method to create imputations under nonignorable models. We do so by simply adding and substracting some amount from the imputations.
...
We only used a shift parameter here. In other applications, scale or shape parameters could be more natural (see e.g. Van Buuren [-@van2012flexible, Ch. 3.9.1]). The calculations are easily adapted to such cases.
