...
We are going to work with the popularity data from Joop Hox -@hox2010multilevel. The variables in this data set are described as follows:
...
The dataset `popNCR` is a variation on the Hox [-@hox2010multilevel] data, where the missingness in the variables is either missing at random (MAR) or missing not at random (MNAR).
...
The latter two methods aggregate level 1 variables at level 2, but in combination with `mice.impute.2l.pan`, allow switching regression imputation between level 1 and level 2 as described in Yucel [-@yucel2008multiple] or Gelman and Hill [-@gelman2007data, p. 541]. For more information on these imputation methods see the help.
